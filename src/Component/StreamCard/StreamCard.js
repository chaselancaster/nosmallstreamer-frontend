...
class StreamCard extends Component {
  ...
  rateStreamer = async (name, vote, user_id) => {
    try {
      ...
      const sendVote = await fetch(
        `${routes.HEROKU}/leaderboard/submit/${name}/${vote}/${user_id}`,
        {
          method: "POST",
          headers: {
  ...
  addToWatchLater = async (name, user_id) => {
    ...
    try {
      const watchLaterCall = await fetch(
        `${routes.HEROKU}/users/watchlater/add/${user._id}/${name}/${user_id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div>
        <li key={streamer.id} className="stream">
          ...
          <div className="stream-info">
            <p className="stream-user-name">{streamer.user_name}</p>
            ...
            <div className="stream-card-buttons">
              <ThumbsUp
                ...
                streamName={streamer.user_name}
                className="thumbs-up"
              />
              <PlusCircle
                onClick={() =>
                  this.addToWatchLater(streamer.user_name, streamer.user_id)
                }
